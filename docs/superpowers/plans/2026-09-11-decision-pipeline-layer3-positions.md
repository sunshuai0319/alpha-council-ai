...
**原方案：交易所侧只挂宽灾难止损（3×stop_distance），软件层管紧的那条。**

当前 as-built 在此基础上增加开仓时静态 2R 交易所 TP；本地仍管理动态止损、结构失效和
超时退出，详见 [`docs/current-architecture.md`](../../current-architecture.md)。
