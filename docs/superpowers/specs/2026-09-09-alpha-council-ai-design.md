...
- 首版优先使用免费外部数据源。
- 首版使用 Clerk 免费版提供登录和 Session，启用邮箱、Google 和 MetaMask；不在首版实现订阅计费、团队协作或 Apple 登录。
...
| 方式 | 决定 | 产品定位 | 主要注意事项 |
| --- | --- | --- | --- |
| 用户名密码 | 有条件保留 | 用户名作为展示名/策略作者名；邮箱作为恢复渠道和主要身份 | 单独的用户名登录会增加找回密码、撞库和账号合并复杂度 |
...
推荐的 Clerk 配置是：**邮箱 + Google 作为主登录方式，MetaMask 作为可选 Web3 登录/绑定方式，用户名只作为用户资料字段而不是独立身份体系**。如果产品验证后确实有用户习惯用户名登录，再将用户名作为登录 identifier，但仍要求账户绑定并验证邮箱。
