...
**为什么动态规则仍在软件层**：交易所的触发单改不了也撤不掉（实测没有 cancel/modify
端点，见 docs/weex-virtual-api.md）。挂紧的止损一碰就直接平仓，移动止损根本没机会执行；
所以有效止损、结构失效和超时规则只能放在本地，触及后下 reduceOnly 市价单。交易所侧
同时保留静态 2R TP 与 3× 宽灾难止损，作为进程故障时的快速退出兜底。
