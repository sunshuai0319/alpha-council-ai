...
export function ConsolePage({ view }: { view: DashboardView }) {
  ...
  if (view === "overview") return <>
    ...
    <div className="hero-grid">
      ...
      <section className="telemetry-panel"><div className="eyebrow">{t("overview.telemetry")}</div><div className="telemetry-price">{latestMarket ? `$${formatNumber(latestMarket.last_price, 2)}` : "—"}</div><div className="telemetry-symbol">{latestMarket?.symbol ?? "BTC-USDT"}<span>{t("overview.lastPrice")}</span></div><div className="telemetry-grid"><span><b>{latestMarket?.funding_rate == null ? "—" : formatPercent(latestMarket.funding_rate, 4)}</b><small>{t("overview.funding")}</small></span><span><b>{latestMarket?.volume_24h == null ? "—" : formatNumber(latestMarket.volume_24h, 0)}</b><small>{t("overview.volume")}</small></span></div></section>
    </div>
